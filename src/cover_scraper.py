...
from src.secondhandsongs_client import (
    scrape_secondhandsongs as scrape_secondhandsongs_client,
)
...
def fetch_work_id_for_recording(
    ...
    try:
        ...
        lookup_resp = requests.get(
            lookup_url, params=lookup_params, headers=headers, timeout=10
        )
            ...
            for rel in relations:
                if rel.get("target-type") == "work" and rel.get("work"):
                    _emit_checked(
                        on_source_checked,
                        source="MusicBrainz",
                        query_kind="work_lookup",
                        query_url=lookup_resp.url,
        ...
        _emit_checked(
            ...
            source="MusicBrainz",
            query_kind="work_lookup",
            query_url=lookup_resp.url if lookup_resp is not None else lookup_url,
            result_count=0,
        )
...
def fetch_covers_for_work(
    ...
    url = "https://musicbrainz.org/ws/2/recording"
    params = {"work": work_id, "inc": "artists", "fmt": "json", "limit": 100}
    headers = {"User-Agent": USER_AGENT}
    covers = []
    try:
        time.sleep(1.1)
        ...
        if resp.status_code == 200:
            ...
            for rec in recordings:
                ...
                artist_name = (
                    artist_credit[0].get("name", "Unknown")
                    if artist_credit
                    else "Unknown"
                )
...
def scrape_covers(
    ...
    # 1. MusicBrainz
    work_id = fetch_work_id_for_recording(
        title, artist, on_source_checked=on_source_checked
    )
    if work_id:
        covers.extend(
            fetch_covers_for_work(
                work_id,
                title,
                artist,
                original_year,
                on_source_checked=on_source_checked,
            )
        )

    # 2. cover.info
    covers.extend(
        scrape_cover_info(title, artist, original_year, callback=on_source_checked)
    )

    # 3. SecondHandSongs
    covers.extend(
        scrape_secondhandsongs(
            title, artist, original_year, on_source_checked=on_source_checked
        )
    )
    ...
    for cover in covers:
        ...
        if key not in unique_covers:
            ...
            if cover.get("musicbrainz_recording_id") and not unique_covers[key].get(
                "musicbrainz_recording_id"
            ):
                unique_covers[key]["musicbrainz_recording_id"] = cover[
                    "musicbrainz_recording_id"
                ]
