...
def find_similar_recordings(
    ...
    # Check if antigravity table exists
    cursor.execute("SELECT count(name) FROM sqlite_master WHERE type='table' AND name='antigravity_mood_event_suggestions'")
    has_suggestions = cursor.fetchone()[0] > 0

    if has_suggestions:
        query = """
            SELECT r.recording_id, r.title, r.artist, r.bpm, r.key, r.genre,
                   COALESCE(r.mood_tags, '') || ',' || COALESCE(s.suggested_mood_tags, '') as mood_tags,
                   COALESCE(r.event_tags, '') || ',' || COALESCE(s.suggested_event_tags, '') as event_tags,
                   COALESCE(r.situation_tags, '') || ',' || COALESCE(s.suggested_situation_tags, '') as situation_tags
            FROM recordings r
        ...
        query = """
            SELECT recording_id, title, artist, bpm, key, genre,
                   mood_tags, event_tags, situation_tags
            FROM recordings
            WHERE recording_id {} ?
        """

    # Fetch target
    cursor.execute(query.format("="), (target_id,))
    ...
    target_tags = set()
    for tag_col in ['mood_tags', 'event_tags', 'situation_tags']:
